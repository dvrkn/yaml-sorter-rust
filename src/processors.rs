...
pub fn process_yaml<'a>(doc: Cow<'a, Yaml>, config: &Yaml) -> Cow<'a, Yaml> {
    match &*doc {
        Yaml::Array(v) => {
            if let Some(sort_key) = config["sortKey"].as_str() {
                ...
                for x in &mut new_v {
                    *x = process_yaml(Cow::Borrowed(x), config).into_owned();
                }
        ...
        Yaml::Hash(h) => {
            if let Some(pre_order_vec) = config["preOrder"].as_vec() {
                ...
                for (_, v) in &mut new_h {
                    *v = process_yaml(Cow::Borrowed(v), config).into_owned();
                }
